...
class EWRLSRidge(object):
    ...
    def cross_validation(
        ...
        res = gp_minimize(
            lambda x: cv_objective(self, x[0], x[1]),  # the function to minimize
        ...
        if verbose:
            LOGGER.info(
                "Optimal CV Error at span = {}".format(self.span)
    ...
    def _initialize_predictions(self):
        ...
        self.nobs = 0
        self.nobs_total = 0
        self.y = StateVariable(
            name="y", col_header="y(t)", method="normal", save_history=True
        )
    ...
    def _run_update(self, y_t: np.ndarray, x_t: np.ndarray, save=True):
        ...
        self.beta.update(
            self.beta.state + k_gain * prediction_error_t_tminus1, save=save
        )
        # kx1 vector beta_t_t
    ...
    def _gain(self, x_update: np.ndarray) -> np.ndarray:
        ...
        @rtype: np.float
        """
        norm_factor = 1 + (1 / self.forgetting_factor) * x_update.T @ self.p @ x_update
    ...
    def _update_remaining_state_vars(
        self, y_hat_t_tminus1, y_hat_t_t, prediction_error_t_tminus1, save=True
    ):
        """
        ...
        self.mse_adj = (
            self.sse / (self.nobs - self.num_features)
            if self.nobs > self.num_features
            else np.nan
        )
    ...
    def renormalize_data(normalized_features, exog_stdev, exog_mean):
        ...
        const = False
        if (type(normalized_features) is pd.DataFrame) and (
            "const" in normalized_features.columns
        ):
    ...
    def _vec_convert(y):
        if (type(y) == np.float64) or (type(y) == float):
            # convert to (1,) array
            y = np.array([y])
        if type(y) == np.ndarray and np.ndim(y) > 1 and np.min(np.shape(y)) == 1:
            if y.shape[0] == 1:
                y = y[0, :]
            elif y.shape[1] == 1:
                y = y[:, 0]
    ...
    def err_handler(self, type, flag):
        ...
        LOGGER.warn(
            "Floating point error {} ".format(type) + " with flag {}".format(flag)
        )
...
class EWRLSChangePoint(EWRLSRidge):
    def __init__(
        ...
        if change_detector_list != []:
            self.change_detectors = [
                BreakDetector(
                    name=func_str, func=self.detectordict[func_str], params=params
                )
    ...
    def _initialize_changept_state(self):
        ...
        self.cusumsqr = StateVariable(
            name="cusumsqr", forgetting_factor=1.0, save_history=True, method="normal"
        )
    ...
    def _run_update(self, y_t: np.ndarray, x_t: np.ndarray, save=True):
        ...
        self.cusumsqr.update(
            np.mean(self.prediction_error_t_tminus1.total_history[-length:] ** 2)
        )
    ...
    def beta_diff(self, param_tuple=(0.99, 3.5)):
        ...
        l2_norm_diff = np.linalg.norm(
            np.diff(self.beta.total_history), axis=0
        )  # default l2 norm
...
class BreakDetector:
    def __init__(
        self,
        name: str,
        func: Callable[[float, float], bool],
...
class StateVariable(object):
    ...
    def _ewm_update(self, update_data):
        if self.state is np.nan:
            ...
            self.state = (
                self.forgetting_factor * self.state
    ...
    def _ew_std_update(self, update_data):
        if self.state is np.nan:
            ...
            self.state = (
                self.forgetting_factor * self.state**2
                + (1 - self.forgetting_factor) * update_data**2
            ) ** (0.5)
...
class EWRLSBatch(EWRLSRidge):
    ...
    def exp_weights(self, vec_length=None, span=None):
        ...
        self.weights = pd.DataFrame(
            wt_vec.values(), index=wt_vec.keys(), columns=["exp_weights"]
        )
    ...
    def _run_update(self, y_t: np.ndarray, x_t: np.ndarray, save=True):
        ...
        restricted_ols_model = sm.WLS(
            self.y.history, self.x.history.T, self.weights
        )  # elastic net here too
...
class CR_RLS(EWRLSRidge):
    ...
    def __init__(
        ...
        reg_dict = {  # f, subgradient_f, rho (max f(beta) ?)
            ...
            "alt_l1": (
                self.l1_norm,
                self.alt_l1_subgradient,
                MAX_L1NORM * self.num_features,
            ),
            "l0": (
                self.pseudo_l0_norm,
                self.pseudo_l0_subgradient,
        ...
        (self.regularization_term, self.subgradient, self.rho) = reg_dict.get(
            regularization_type, (None, None, 0.0)
        )
    ...
    def __repr__(self):
        return (
            f'CR_RLS("span={self.span}", "l2-regularization={self.regularization}",'
    ...
    def cross_validation(
        ...
        def cv_objective(
            self, span=None, regularization=None, sparse_regularization=None
        ):
            # default to original span and regularization
        ...
        res = gbrt_minimize(
            lambda x: cv_objective(self, x[0], x[1], x[2]),  # the function to minimize
            [
                span_bounds,
                ridge_reg_bounds,
                sparse_reg_bounds,
        ...
        # don't leave at last evaluated point, reset to optimal
        self.reset(
            span=res.x[0], regularization=res.x[1], sparse_regularization=res.x[2]
        )
    ...
    def _run_update(self, y_t: np.ndarray, x_t: np.ndarray, save=True):
        ...
        #    p - p.dot(x_t) * c * x_t.T.dot(P)
        eta = (
            self.sparse_regularization * (1 - self.forgetting_factor)
            if self.scale_sparse
            else self.sparse_regularization
        )
        if self.dynamic_regularization:
            eta = eta * gamma_n
        ...
        beta_update = (
            self.beta.state
            + k_gain * prediction_error_t_tminus1
...
def main():
    ...
    model = EWRLSRidge(
        num_features=2, span=90, regularization=30, history=True, calc_stderr=True
    )
