...
class NavSeries(pd.Series):
    ...
    def periods_per_year(self):
        if len(self.index) >= 2:
            ...
            return np.round(
                365 * 24 * 60 * 60 / x.diff().mean().total_seconds(), decimals=0
            )
        else:
            return 256
    ...
    def summary(self, r_f=0):
        ...
        d["Annua Sharpe Ratio (r_f = {0})".format(r_f)] = self.sharpe_ratio(
            periods=periods, r_f=r_f
        )
...
def build_table(cov_models, pnl_results):
    ...
    for m in pnl_results:
        ...
        full_table.loc[m, "SR"] = round(
            perf["Cumulative_Return"]["Annual Sharpe Ratio (r_f = 0)"], 2
        )
        full_table.loc[m, "AnnRet"] = round(
            perf["Cumulative_Return"]["Annual Return"], 2
        )
        full_table.loc[m, "Vol"] = round(
            perf["Cumulative_Return"]["Annual Volatility"], 2
        )
...
def plotting(pnl_results, cov_models):
    ...
    pnl_fig = go.Figure(
        ...
        layout=go.Layout(
            ...
            yaxis=dict(title="Cumulative Returns (Log-Scale)", type="log"),
        ),
    )
    pnl_fig.show()
...
def annual_return(returns, price_rets="price", trading_days=252):
    ...
    if price_rets == "returns":
        return np.prod((1.0 + v) ** (1.0 / n_years)) - 1.0 if v.size > 0 else np.nan
    else:
        return np.sum(v) * (1.0 / n_years) if v.size > 0 else np.nan
...
def sharpe_ratio(returns, risk_free=0.0, trading_days=252):
    assert trading_days > 0, "Trading days needs to be > 0"
...
def max_drawdown(returns, price_rets="price"):
    ...
    if v.size == 0:
        return np.nan
    if price_rets == "returns":
        cumret = np.concatenate(([100.0], 100.0 * np.cumprod(1.0 + v)))
        maxret = np.fmax.accumulate(cumret)
        return np.nanmin((cumret - maxret) / maxret)
    else:
        cumret = np.concatenate(([1.0], np.cumsum(v)))
        maxret = np.fmax.accumulate(cumret)
...
def max_drawdown_length(returns, price_rets="price"):
    ...
    if price_rets == "returns":
        ...
        cumret = np.concatenate(([1.0], np.cumsum(v)))  # start at one? no matter
        maxret = np.fmax.accumulate(cumret)
...
def omega_ratio(returns, risk_free=0.0, target_return=0.0, trading_days=252):
    assert trading_days > 0, "Trading days needs to be > 0"
    v = _convert_to_array(returns)
    if v.size == 0:
        return np.nan
    return_thresh = (1.0 + target_return) ** (1.0 / trading_days) - 1.0
    v = v - risk_free - return_thresh
    numer = np.sum(v[v > 0.0])
    denom = -np.sum(v[v < 0.0])
    return numer / denom if denom > 0.0 else np.nan


def sortino_ratio(returns, target_return=0.0, trading_days=252):
    assert trading_days > 0, "Trading days needs to be > 0"
    v = _convert_to_array(returns)
    if v.size == 0:
        return np.nan
    v = v - target_return
    downside_risk = np.sqrt(np.mean(np.square(np.clip(v, np.NINF, 0.0))))
...
def tail_ratio(returns):
    v = _convert_to_array(returns)
    return (
        np.abs(np.percentile(v, 95.0)) / np.abs(np.percentile(v, 5.0))
        if v.size > 0
        else np.nan
    )
...
def common_sense_ratio(returns):
    ...
    if v.size == 0:
        return np.nan
    return tail_ratio(returns) * np.sum(v[v > 0.0]) / np.abs(np.sum(v[v < 0.0]))
